        ...
        <div class="divPanel notop page-content">
            ...
            <!--Edit Main Content Area here-->
                <div class="span12" id="divMain">
					...
					<p><strong>
					<form name="f1" action="InternalSeating.php" method="post">
					  <table width="463" border="0" align="center">
                        <tr>
                          <td width="140">Date(DD-MM-YYYY):</td>
...
$q10="create table `$d` (enrollment_id varchar(12) primary key,department varchar(6),room_no varchar(8),series varchar(2),number int(4))";
$r10=mysql_query($q10);
$r9=mysql_query($q9);
$q1="select semester from timetable where department='".$d."' and start_date='".$a."' ";
$r1=mysql_query($q1);

	while($arr=mysql_fetch_row($r1))		
	{
		foreach($arr as $sem)
		{
		$i++;
		$q3="select * from room where department='".$d."'";
		$r3=mysql_query($q3);
		$q2="select enrollment_no from studentdetails where department='".$d."' and semester='".$sem."'";
		$r2=mysql_query($q2);
			while($row1=mysql_fetch_array($r3))
			{
			$q4="select * from `$d` where room_no='".$row1['room_no']."' and  department='".$row1['department']."'" ;
			$r4=mysql_query($q4);
			$z=mysql_num_rows($r4);
				if($i%2==0)
				{
					if($z < $row1['max_seats'] && $row1['fillB']!=$row1['max_seats']/2  )
					{
						...
						while(($count<=$var) && ($row=mysql_fetch_row($r2)))
						{
											
							foreach($row as $id)
						
							{
							$qr4="insert into `$d` values('".$id."','".$row1['department']."','".$row1['room_no']."','B','".$count."')";
	...
	//echo "$id";GET ENROLLMENT ID
							...
							$qr4="insert into `$d` values('".$id."','".$row1['department']."','".$row1['room_no']."','A','".$count."')";
